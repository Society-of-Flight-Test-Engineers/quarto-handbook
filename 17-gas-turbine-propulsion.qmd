...
| Turbojet                              |  Turboprop                             |
|                                       |                                        |
| ![](media/17/turbojet.png)            | ![](media/17/turboprop.png)            |
+---------------------------------------+----------------------------------------+
| Low Bypass Turbofan                   |  High Bypass Turbofan                  |
|                                       |                                        |
| ![](media/17/low-bypass-turbofan.png) | ![](media/17/high-bypass-turbofan.png) |
+---------------------------------------+----------------------------------------+
...
optimization for most flight regimes. Turbofan **advantages include:** 
...
1. The inlet and compressor (engine stations 0 to 3) isentropically compress the 
air.
2. The combustor (engine stations 3 to 4) provides isobaric heating.
3. The turbine and nozzle (engine stations 4 to 9) isentropically expand the air 
to free stream jet.
4. Free stream exhaust jet is at a higher velocity and temperature (and entropy) 
than the inlet.
...
+:-------------------------------------------+:-------------------------:+
| Turbines extract energy from the combustor | ![][17005]![][17006]      |
...
| turbine powers the low pressure compressor.|                           |
+--------------------------------------------+---------------------------+
...
1. Overall: Integrated System Utility (Does it meet the users' needs?). 
Topics include adequate engine bay ventilation, anti-ice, gun or gas ingestion, 
and inlet compatibility.
2. Performance: The ability to produce thrust at a prescribed level with a 
...
3. Operability: The ability to resist or recover from an engine instability. 
...
4. Response: The ability to change thrust conditions within a prescribed time in 
response to a commanded change.
5. Life/Durability: The ability to withstand extended operating conditions 
...
\end{equation*}
  | 
\begin{equation*}
{\dot{m}}_{\mathrm{ci}} = \frac{{\dot{m}}_{i}\sqrt{\theta_{i}}}{\delta_{i}}
\end{equation*}
 |
|**Fuel flow rate**     |
\begin{equation*}
...
\end{equation*}
  | 
\begin{equation*}
{\dot{m}}_{\mathrm{cf}} = \frac{{\dot{m}}_{f}}{\delta_{2}\sqrt{\theta_{2}}}
\end{equation*}
 |
|**Rotational speed**   |
\begin{equation*}
...
\end{equation*}
              |
\begin{equation*}
N_{\mathrm{ci}} = \frac{N}{\sqrt{\theta_{i}}}
\end{equation*}
 |
|**Thrust**             |
\begin{equation*}
...
### Pressure and Temperature Relationships with Flight Parameters
...
\begin{equation*}
\text{Thrust} = \frac{{\dot{m}}_{0}(V_{9} - V_{0})}{g_{c}}
\end{equation*}
                                                                          |
+----------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| High Bypass Turbofan (Separate Streams)            | 
\begin{equation*}
\text{Thrust} = \frac{{\dot{m}}_{\text{fan}}}{g_{c}}(V_{\text{fan}} - V_{0}) + \frac{{\dot{m}}_{\text{core}}}{g_{c}}(V_{9} - V_{0})
\end{equation*}
 |
+----------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
### Compressor operation during accelerations

![](media/17/eng-accel-compressor-map.png)
...
### Compressor operation during decelerations

![](media/17/eng-decel-compressor-map.png)
...
### Bleed air impacts during engine starts

![](media/17/eng-start-compressor-map.png)
...
### Variable Stator vane cambering effects

![](media/17/stator-rotor-velocity.png)
...
![](media/17/variable-geometry-compressor-map.png)
...
### Compressor impacts due to nozzle area for [turbojet]{.underline} engines

![](media/17/turbojet-open-close-nozzle-compressor-map.png)
...
### Compressor impacts due to nozzle area for [turbofan]{.underline} engines

(mixed flow, proximate splitter)

![](media/17/turbofan-open-close-nozzle-compressor-map.png)
...
![](media/17/combustion-stability.png)
...
(Reference SAE ARP-755 *Aircraft Propulsion System Performance Station Designation and Nomenclature* 
...
1. kinetic compression (inlet/diffuser)
2. mechanical compression/work addition/fluidic compression (compressor/propeller)
3. heat addition or exchange (combustor/augmentor/heat exchanger)
4. mechanical expansion/work extraction (turbine)
5. kinetic expansion (nozzle)
6. mixing (mixer/ejector/eductor)
...
1.  Incremental (or sub) stations may be indicated with suffix nomenclature 
(e.g. 2.5 to indicate fan discharge on a dual spool compression system).

2.  There are a multitude of variations on this theme. SAE ARP-755 includes 
descriptions for most turbine engine configurations.

### Key Propulsion Terminology
...
### Aircraft and Associated Engines
...
### Additional Propulsion Resources
...
<http://standards.sae.org/power-propulsion/engines/gas-turbines/standards/>
  ...
  [17008]: media/17/image13.png {width="3.96579615048119in" height="3.798507217847769in"}
